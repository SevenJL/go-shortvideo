...
func (w *Worker) Handle(ctx context.Context, task Task) error {
	...
	for _, r := range results {
		...
		outputPaths[r.Resolution.Name] = r.OutputPath
	...
	// 7. 如果有 OSS 客户端，上传到 OSS 并把最终 CDN/OSS URL 写回数据库。
	if w.oss != nil && !w.oss.IsLocal() {
		if err := w.uploadToOSS(task.VideoID, outputPaths, playURLs, coverPath, &coverURL); err != nil {
			_ = w.updater.UpdateStatus(ctx, task.VideoID, model.VideoFailed)
			return err
		}
	}
...
// uploadToOSS 上传转码产物到 OSS，并把 playURLs/coverURL 替换为最终访问地址。
func (w *Worker) uploadToOSS(videoID int64, outputPaths map[string]string, playURLs map[string]string, coverPath string, coverURL *string) error {
	ossPrefix := fmt.Sprintf("videos/%d", videoID)
	// 上传各分辨率
	for res, localPath := range outputPaths {
		ossKey := fmt.Sprintf("%s/%s.mp4", ossPrefix, res)
		if err := w.oss.PutObject(ossKey, localPath); err != nil {
			return fmt.Errorf("oss upload %s: %w", res, err)
		}
		playURLs[res] = w.oss.ObjectURL(ossKey)
	}
	// 上传封面
	if coverPath != "" {
		ossKey := fmt.Sprintf("%s/cover.jpg", ossPrefix)
		if err := w.oss.PutObject(ossKey, coverPath); err != nil {
			return fmt.Errorf("oss upload cover: %w", err)
		}
		*coverURL = w.oss.ObjectURL(ossKey)
	}
	return nil
}
